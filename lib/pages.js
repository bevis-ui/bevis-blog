 ...
 * @returns {Promise~String} built HTML
 */
function buildPageUsingAssets(pageName, path, data, pages, templates) {
    var root = (path ? (new Array(path.split('/').length + 1)).join('../').replace(/\/$/, '') : '.');
    var assetsPath = root + '/build/build.min';
...
function buildPage(pageName, path, data) {
    return vow.all([
            ...
            return buildPageUsingAssets(pageName, path, data, pages, templates);
        });
}
...
module.exports.buildPageUsingAssets = buildPageUsingAssets;
module.exports.requireAsset = requireAsset;
